...
with patch('som.core.common.som_uid'):
    with patch('som.core.common.som_gid'):
        from som.core.task import *
        from som.core.utils import SomDBQuery


def test_som_db_query_task__init__():
    """Check if SomDBQueryTask is properly initialized."""
    with patch('som.core.utils.SomDBQuery.__init__') as mock_wdbq, \
            patch('som.core.utils.SomDBBackend.__init__', return_value=None) as mock_wdb_backend:
        SomDBQueryTask()
...
@patch('som.core.som_socket.SomSocket.receive', return_value=b'"{\'test\':\'test\'}"')
@patch('som.core.som_socket.SomSocket.send')
@patch('som.core.som_socket.SomSocket.close')
def test_send_to_tasks_socket(mock_close, mock_send, mock_receive, mock_connect):
    ...
    ----------
        mock_send: MagicMock
            Mock of SomSocket's method send.
        mock_connect: MagicMock
            Mock of SomSocket's method connect.
        mock_close: MagicMock
            Mock of SomSocket's method close.
        mock_receive: MagicMock
            Mock of SomSocket's method receive.
    """
    command = {'test': 'test'}
...
def test_send_to_tasks_socket_ko():
    ...
    with pytest.raises(SomInternalError, match=".* 1121 .*"):
        send_to_tasks_socket({'test': 'test'})


def test_som_db_query_task__final_query():
    """Check if SomDBQueryTask's method _final_query works properly."""
    with patch('som.core.utils.SomDBBackend.__init__', return_value=None), \
            patch('som.core.utils.SomDBQuery._default_query', return_value='test'):
        wdbq_task = SomDBQueryTask()
        assert wdbq_task._final_query() == f'test WHERE task_id IN (test) LIMIT :limit OFFSET :offset'


def test_som_db_query_task__format_data_into_dictionary():
    """Check that SomDBQueryTask's method _format_data_into_dictionary works properly."""
    data = [
...
def test_som_db_query_task__process_filter(field_name, field_filter, q_filter):
    """Check that SomDBQueryTask's method _process_filter works properly."""
    with patch('som.core.utils.SomDBBackend.__init__', return_value=None), \
            patch('som.core.utils.SomDBQuery._default_query', return_value='test'):
        wdbq_task = SomDBQueryTask()

    with patch.object(SomDBQuery, '_process_filter') as mock_sup_proc:
