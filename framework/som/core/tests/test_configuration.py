...
from som.core.common import OSSEC_CONF, REMOTED_SOCKET
...
def mock_som_path():
    with patch('som.core.common.WAZUH_PATH', new=os.path.join(parent_directory, tmp_path)):
        yield
...
def test_agentconf2json():
    xml_conf = configuration.load_som_xml(
...
def test_get_ossec_conf():
    with patch('som.core.configuration.load_som_xml', return_value=Exception):
        with pytest.raises(SomError, match=".* 1101 .*"):
...
def test_get_agent_conf():
    with pytest.raises(SomError, match=".* 1710 .*"):
        configuration.get_agent_conf(group_id='noexists')

    with patch('som.core.common.SHARED_PATH', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        with pytest.raises(SomError, match=".* 1006 .*"):
...
def test_get_agent_conf_multigroup():
    ...
    with patch('som.core.common.MULTI_GROUPS_PATH', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        with pytest.raises(SomError, match=".* 1006 .*"):
...
def test_get_file_conf():
    with patch('som.core.common.SHARED_PATH', new=os.path.join(parent_directory, tmp_path, 'noexists')):
        with pytest.raises(SomError, match=".* 1710 .*"):
    ...
    with patch('som.core.common.SHARED_PATH', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        ...
        ar_list = ['restart-ossec0 - restart-ossec.sh - 0', 'restart-ossec0 - restart-ossec.cmd - 0',
                   'restart-som0 - restart-ossec.sh - 0', 'restart-som0 - restart-ossec.cmd - 0',
                   'restart-som0 - restart-som - 0', 'restart-som0 - restart-som.exe - 0']
...
def test_parse_internal_options():
    ...
    with patch('som.core.common.INTERNAL_OPTIONS_CONF',
        ...
        with patch('som.core.common.LOCAL_INTERNAL_OPTIONS_CONF',
                   new=os.path.join(parent_directory, tmp_path, 'configuration/local_internal_options.conf')):
            with pytest.raises(SomInternalError, match=".* 1108 .*"):
                configuration.parse_internal_options('ossec', 'python')


def test_get_internal_options_value():
    with patch('som.core.configuration.parse_internal_options', return_value='str'):
        with pytest.raises(SomError, match=".* 1109 .*"):
            configuration.get_internal_options_value('ossec', 'python', 5, 1)

    with patch('som.core.configuration.parse_internal_options', return_value='0'):
        with pytest.raises(SomError, match=".* 1110 .*"):
            configuration.get_internal_options_value('ossec', 'python', 5, 1)

    with patch('som.core.configuration.parse_internal_options', return_value='1'):
        assert configuration.get_internal_options_value('ossec', 'python', 5, 1) == 1


@patch('som.core.configuration.common.som_gid')
@patch('som.core.configuration.common.som_uid')
@patch('builtins.open')
def test_upload_group_configuration(mock_open, mock_som_uid, mock_som_gid):
    with pytest.raises(SomError, match=".* 1710 .*"):
    ...
    with patch('som.core.common.SHARED_PATH', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        with patch('som.core.configuration.tempfile.mkstemp', return_value=['mock_handle', 'mock_tmp_file']):
            with patch('som.core.configuration.open'):
                with pytest.raises(SomInternalError, match=".* 1743 .*"):
                    configuration.upload_group_configuration('default', "<agent_config>new_config</agent_config>")
            with patch('som.core.configuration.open', return_value=Exception):
                with pytest.raises(SomError, match=".* 1113 .*"):
                    configuration.upload_group_configuration('default', "<agent_config>new_config</agent_config>")
            with patch('builtins.open'):
                with patch('som.core.configuration.subprocess.check_output', return_value=True):
                    with patch('som.core.utils.chown', side_effect=None):
                        with patch('som.core.utils.chmod', side_effect=None):
                            with patch('som.core.configuration.safe_move'):
                                assert isinstance(configuration.upload_group_configuration('default',
                                                                                           "<agent_config>new_config"
                                                                                           "</agent_config>"),
                                                  str)
                            with patch('som.core.configuration.safe_move', side_effect=Exception):
                                with pytest.raises(SomInternalError, match=".* 1016 .*"):
                                    configuration.upload_group_configuration('default',
                                                                             "<agent_config>new_config</agent_config>")
            with patch('som.core.configuration.subprocess.check_output',
                       side_effect=subprocess.CalledProcessError(cmd='ls', returncode=1, output=b'ERROR')):
                with patch('som.core.configuration.re.findall', return_value=None):
                    with pytest.raises(SomError, match=".* 1115 .*"):
                        configuration.upload_group_configuration('default', "<agent_config>new_config</agent_config>")
                with patch('som.core.configuration.re.findall', return_value='1114'):
                    with patch('os.path.exists', return_value=True):
                        with patch('som.core.configuration.remove') as mock_remove:
                            with pytest.raises(SomError, match=".* 1114 .*"):
                                configuration.upload_group_configuration('default',
                                                                         "<agent_config>new_config</agent_config>")
...
@patch('som.core.configuration.common.som_uid')
@patch('builtins.open')
@patch('som.core.configuration.safe_move')
def test_upload_group_file(mock_safe_move, mock_open, mock_som_uid, mock_som_gid):
    with pytest.raises(SomError, match=".* 1710 .*"):
        configuration.upload_group_file('noexists', 'given', 'noexists')

    with patch('som.core.configuration.os_path.exists', return_value=True):
        with pytest.raises(SomError, match=".* 1112 .*"):
    ...
    with patch('som.core.common.SHARED_PATH', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        with pytest.raises(SomError, match=".* 1111 .*"):
...
@pytest.mark.parametrize("agent_id, component, socket, socket_dir, rec_msg", [
    ...
    ('000', 'som-db', 'wdb', 'db', {"error": 0, "data": {"enabled": "yes"}}),
...
@patch('som.core.som_socket.create_som_socket_message')
@patch('os.path.exists')
@patch('som.core.common.WAZUH_PATH', new='/var/ossec')
def test_get_active_configuration(mock_exists, mock_create_som_socket_message, agent_id, component, socket,
                                  socket_dir, rec_msg):
    """This test checks the proper working of get_active_configuration function."""
    sockets_json_protocol = {'remote', 'analysis', 'wdb'}
    config = MagicMock()

    socket_class = "SomSocket" if socket not in sockets_json_protocol or agent_id != '000' else "SomSocketJSON"
    with patch(f'som.core.som_socket.{socket_class}.close') as mock_close:
        with patch(f'som.core.som_socket.{socket_class}.send') as mock_send:
            with patch(f'som.core.som_socket.{socket_class}.__init__', return_value=None) as mock__init__:
                with patch(f'som.core.som_socket.{socket_class}.receive',
                           return_value=rec_msg.encode() if socket_class == "SomSocket" else rec_msg) as mock_receive:
                    ...
                    if socket_class == "SomSocket":
                        mock_send.assert_called_with(f"getconfig {config}".encode() if agent_id == '000' else \
                                                         f"{agent_id} {component} getconfig {config}".encode())
                    else:  # socket_class == "SomSocketJSON"
                        mock_create_som_socket_message.assert_called_with(origin={'module': ANY},
                                                                            command="getconfig",
                                                                            parameters={'section': config})
                        mock_send.assert_called_with(mock_create_som_socket_message.return_value)
...
@pytest.mark.parametrize('agent_id, component, config, socket_exist, socket_class, expected_error, expected_id', [
    ...
    ('000', 'test_component', None, ANY, 'SomSocket', SomError, 1307),  # No configuration
    ('000', None, 'test_config', ANY, 'SomSocket', SomError, 1307),  # No component
    ('000', 'test_component', 'test_config', ANY, 'SomSocket', SomError, 1101),  # Component not in components
    ('001', 'syscheck', 'syscheck', ANY, 'SomSocket', SomError, 1116),  # Cannot send request
    ('001', 'syscheck', 'syscheck', ANY, 'SomSocket', SomError, 1117),  # No such file or directory

    # Checks for 000 - Simple messages
    ('000', 'syscheck', 'syscheck', False, 'SomSocket', SomError, 1121),  # Socket does not exist
    ('000', 'syscheck', 'syscheck', True, 'SomSocket', SomInternalError, 1121),  # Error connecting with socket
    ('000', 'syscheck', 'syscheck', True, 'SomSocket', SomInternalError, 1118),  # Data could not be received

    # Checks for 000 - JSON messages
    ('000', 'request', 'global', False, 'SomSocketJSON', SomError, 1121),  # Socket does not exist
    ('000', 'request', 'global', True, 'SomSocketJSON', SomInternalError, 1121),  # Error connecting with socket
    ('000', 'request', 'global', True, 'SomSocketJSON', SomInternalError, 1118),  # Data could not be received

    # Checks for 001
    ('001', 'syscheck', 'syscheck', ANY, 'SomSocket', SomInternalError, 1121),  # Error connecting with socket
    ('001', 'syscheck', 'syscheck', ANY, 'SomSocket', SomInternalError, 1118)  # Data could not be received
...
def test_get_active_configuration_ko(mock_exists, agent_id, component, config, socket_exist, socket_class,
    ...
    with patch(f'som.core.som_socket.{socket_class}.__init__',
               return_value=MagicMock() if expected_id == 1121 and socket_exist else None):
        with patch(f'som.core.som_socket.{socket_class}.send'):
            with patch(f'som.core.som_socket.{socket_class}.receive',
                ...
                with patch(f'som.core.som_socket.{socket_class}.close'):
...
def test_write_ossec_conf():
    content = "New config"
    with patch('som.core.configuration.open', mock_open()) as mocked_file:
...
def test_write_ossec_conf_exceptions():
    with patch('som.core.configuration.open', return_value=Exception):
        with pytest.raises(SomError, match=".* 1126 .*"):
...
@patch('som.core.configuration.get_ossec_conf')
...
def test_update_check_is_enabled_exceptions(error_id, value):
    ...
    with patch('som.core.configuration.get_ossec_conf', side_effect=SomError(error_id), return_value=value):
        if value is not None:
            assert configuration.update_check_is_enabled() == value
        else:
            with pytest.raises(SomError, match=f'.* {error_id} .*'):
